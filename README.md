...
> **What it is not:** a diagnosis or treatment tool. CarePlus never diagnoses, prescribes, changes medicines, determines causation or declares a medicine safe. Those words are enforced in the system prompt, the tool surface and the routing engine, not just the README.

| Judging lens | Where to look |
| --- | --- |
| Technical execution | [Architecture](#architecture) · `src/agent/` (tool-calling coordinator) · `src/core/policy/` (deterministic routing) · `src/specialists/txgemma/` |
| Reliability & evaluation | [Reliability testing](#reliability-testing) — `npm run eval`, 8 end-to-end scenarios with assertions, all passing |
| Usefulness | [Project overview](#project-overview) — the patient's job is *done*, not answered |
| Originality | Provenance-tagged facts · TxGemma as a bounded molecular specialist · Slack as an exception console, not a chat mirror |
| Demo clarity | [Demo video](#demo-video) · [Two-minute walkthrough](#the-two-minute-walkthrough) |
...
CarePlus is the assistant a clinic hands its patients. In one conversation it:

1. **Reconstructs history** from the clinic record (Firestore) — prior appointments, patient-reported medicines, care-plan tasks, accessibility preferences, prior agent conversations, staff corrections.
2. **Asks for missing evidence** and reads it — a photo of a bottle, blister pack or discharge summary goes through Gemini vision OCR. Extracted text is shown back with a confidence score and **does not become record until the patient confirms it**.
3. **Resolves the medicine** against authoritative sources: RxNorm (brand → generic → ingredients), PubChem (canonical SMILES, formula, CID) and openFDA (official label: warnings, contraindications, adverse reactions).
4. **Runs TxGemma** — Google's open therapeutics model, deployed on a dedicated Vertex AI endpoint — on the confirmed molecule's SMILES. Tasks (ClinTox, blood-brain-barrier, hERG, skin reaction, CYP inhibition…) are selected from what the patient reported. The output is labelled everywhere as a *research signal for the clinic team*, is compared against the official label, and is never shown as a clinical verdict.
5. **Routes deterministically.** A clinic-authored policy engine — not the LLM — decides the tier:
   - **Green** — fully autonomous: book allowed appointment types, send confirmations, record preferences, transfer records.
   - **Amber** — reversible preparation, then review: hold the slot, gather evidence, open one staff review, keep the patient informed. Triggers include prior adverse experience, uncertain medicine match, a new medicine alongside a high-monitoring one (e.g. warfarin), post-discharge changes, conflicting records, TxGemma-vs-label disagreement.
   - **Red** — clinic-authored red-flag rules screen every message *before* the LLM sees it. Ordinary workflow stops, the approved urgent instruction is shown with a call action, staff are alerted.
6. **Takes real actions**: dependency-aware bookings in Google Calendar (blood test → results window → follow-up), SMS to the patient, evidence to Cloud Storage, a Block Kit review card to Slack with buttons staff can actually press.
7. **Closes the loop with staff.** Staff ask the Slack bot (or the web console) *"Why was CP-1043 routed for review?"* and get the answer from the case's decision trace, with every fact tagged by provenance. They click **Ask patient: date of last dose** and the question appears in the patient's chat and phone; the answer comes back to the review.
...
| Prospective patient (no record) | `/portal/prospective` | Conversational intake → provisional, clearly-labelled self-reported record → first visit booked |
| Clinic staff | Slack private channel or `/staff` | Exceptions only. Review cards with actions; ask questions of any case; decision trace |
| Research coordinator | `/research` | De-identified study pre-screens with consent status; recurring molecular signals |

### Provenance is first-class

Every fact carries one of `patient_reported · document_extracted · database_verified · model_predicted · staff_confirmed · clinic_policy`. The UI shows the tag; the coordinator reasons with it; staff can ask "which facts came from a document and which from the patient?" and get two lists. A prospective patient's record is visibly labelled *Provisional · self-reported* until the clinic verifies it.
...
3. In one turn CarePlus resolves the medicine (RxNorm → PubChem → openFDA), runs TxGemma on both ingredients, hits **amber** (prior adverse experience), **holds** a video medication review with her usual GP in Google Calendar, sends the SMS, stores the photo in Cloud Storage and opens **one** staff review.
4. In Slack (or `/staff`) ask *"Why was this routed for review?"* → the answer comes from the trace. Click **Ask patient: date of last dose** → the question lands on Maya's phone. She answers; the reviewer sees it. **Confirm appointment** → hold becomes booking; Maya gets the confirmation.

Other personas exercise the other paths: **Arjun** (three dependency-aware bookings incl. wheelchair-accessible physio, fully autonomous), **Lucia** (Portuguese; routine asthma review booked *and* a medication review held for a prior rash), **Tom** (post-discharge, doesn't know what to book), **Grace** (research pre-screen with consent + records transfer), **I'm new here** (prospective intake).

---

## External apps used

CarePlus takes actions across these systems. "Live" means the code path runs against the real service in the demo; nothing below is a stub.
...
| 2 | **Google Cloud Storage** | Evidence store for uploaded photos/PDFs; case record keeps only the `gs://` pointer; UI gets short-lived V4 signed URLs | Upload on ingest, signed read for the portal and staff console | Live |
...
Setup for each is in [Setup instructions](#setup-instructions). The home page (`/`) shows which integrations are connected in the running instance.
...
- Node 20+
- A Google Cloud project with **Firestore**, **Cloud Storage**, **Vertex AI** and **Google Calendar** APIs enabled, and a service-account key (or `gcloud auth application-default login`)
- An OpenAI API key
...
| `FAST_MODEL`, `VISION_MODEL` (`gemini-3.1-flash-lite`) | Vertex-served Gemini for OCR + classifier |
...
**TxGemma:** in Vertex AI Model Garden open *TxGemma* → *Deploy* `txgemma-2b-predict` (one L4 is enough). Then:
...
The Firestore database is seeded automatically with the synthetic clinic and five synthetic patients on first request.
...
| `npm run eval [scenario…]` | End-to-end scenario harness with assertions (see below) |
...
- **The LLM does not route.** It must call `evaluate_routing`; the deterministic policy engine (`src/core/policy/policyEngine.ts`) returns the tier and what may be booked. Clinic rules are data (`src/data/clinic.ts`), not prompt text.
- **Red flags are screened before the model sees the message** (`src/core/policy/redFlags.ts`): keyword rules, plus a fast classifier only to distinguish a *historical* mention ("I had chest pain last year, it was reflux") from a current one — so safety never depends on the coordinator behaving.
- **Nothing extracted from an image becomes record until the patient confirms it.** Extraction confidence is stored and shown.
- **TxGemma is bounded**: it only ever receives a SMILES string, its output is typed (`TxGemmaSignal`), labelled `model_predicted`, compared with the official label, and a disagreement is itself an amber trigger. Regression tasks were removed from auto-selection after evaluation showed low-quality outputs at 2B.
- **Idempotent side effects**: one open staff review per case (later findings update it rather than duplicate it); re-booking the same appointment type cancels the superseded hold in Calendar; SMS only when something changed for the patient; portal URLs in SMS are generated by the system, never by the model.
- **Bounded tool loop** with a forced summary turn if the budget is exhausted, so the patient always gets a real reply.
- **Retries with backoff** on Vertex 429/503; graceful degradation when an integration is missing (virtual phone, web staff console, in-memory store) — visible on the home page rather than silent.
- **Firestore-safe records**: evidence goes to Cloud Storage and only the pointer is stored, keeping case documents well under the 1 MiB limit (a full Maya case is ~30 KB).

### Measured: `npm run eval`

`scripts/eval.ts` drives the **real coordinator, real tools and real external APIs** (Vertex Gemini, TxGemma, RxNorm/PubChem/openFDA, Calendar in dry-run) through scripted multi-turn patient conversations on a fresh store, then asserts on the *outcomes* — cards rendered, tier chosen, appointments held/booked, reviews created, TxGemma tasks selected, SMS content — cumulatively across turns.

| Scenario | Path exercised | Key assertions |
| --- | --- | --- |
| `maya` | Prior adverse experience → photo → confirm → amber | `evidence_request` card on turn 1; extraction confirm card; RxNorm exact match; ClinTox + BBB signals; **held** video review with usual GP; exactly one staff review; SMS carries no clinical detail |
...
| `tom` | Post-discharge, unsure what to book | Discharge PDF extracted; medication change → amber; correct appointment type chosen |
...
| `grace` | Research interest + records transfer | Study pre-screen with explicit consent; records-transfer task; no eligibility claim made |
...
**Current result: 8 / 8 scenarios passing.** Each run prints the agent's replies, cards, decision trace and every external action so regressions in behaviour — not just crashes — are visible. The harness was the primary instrument for tuning routing and prompts; the fixes it drove (duplicate reviews, invented URLs, missing upload control, over-eager SMS, symptom-to-label matching) are all now assertions.
...
- End-to-end browser run of the Maya flow against **live Firestore + Cloud Storage + Calendar + Vertex**: case, document pointer, appointment, review and SMS all present in Firestore; photo present in the bucket; staff console reads the same record.
...
**▶ Demo (≤ 2 min):** _link to be added at submission_

What the video shows, in order: Maya's request → history found → bottle photo read and confirmed → RxNorm/PubChem/openFDA + TxGemma in one turn → amber routing → Calendar hold + SMS on the phone → Slack/staff review card → *"Why was this routed?"* answered from the trace → *Ask patient* lands on the phone → staff confirms → booking receipt.
